...
namespace InvoiceCheckApp
{
    ...
    {
        static Dictionary<string, Invoice> _DicOfAllInvoice;
        static Dictionary<string, string> _DicOfThreeNumbersOfAllPrize = new Dictionary<string, string>();
        static string CurrentChooseIntervalStr = "";
        ...
        private void Form1_Load(object sender, EventArgs e)//window load
        {
            ...
            _DicOfAllInvoice = _ReturnInvoice.GetInvoiceFromUrl();
            var IntervalList = _DicOfAllInvoice.Select(m => m.Key).ToList();

            OneBtn.Text = IntervalList[0];
            SecondBtn.Text = IntervalList[1];
            ThirdBtn.Text = IntervalList[2];
            SelectInterval((object)OneBtn, null);
        ...
        {
            _DicOfThreeNumbersOfAllPrize.Clear();

            CurrentChooseIntervalStr = ((System.Windows.Forms.Button)sender).Text;

            var SelectIntervalOfAllPrize = from s in _DicOfAllInvoice
            ...
            foreach (var PrizeNumber in SelectIntervalOfAllPrize)
            {
        ...
        private void InputNumberBox_KeyPress(object sender, KeyPressEventArgs e)//only input number
        {
        ...
        private void InputNumberBox_TextChanged(object sender, EventArgs e)
        {
            ...
            {
                var Query = _DicOfThreeNumbersOfAllPrize.Where(s => s.Value == InputNumberBox.Text).Select(m => m.Key).FirstOrDefault();
        ...
        {
            var Query = _DicOfAllInvoice.Where(m => m.Key == CurrentChooseIntervalStr).FirstOrDefault();

            string isVerySpecialPrize, isSpecialPrize;
            List<string> isJackPot, isSixthPrize;

            isVerySpecialPrize = Query.Value.VerySpecialPrize;
            isSpecialPrize = Query.Value.SpecialPrize;
            isJackPot = Query.Value.JackPot;
            isSixthPrize = Query.Value.SixthPrize;
